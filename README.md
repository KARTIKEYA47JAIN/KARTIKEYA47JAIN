<div align="center">

<img src="https://readme-typing-svg.herokuapp.com?font=JetBrains+Mono&weight=500&size=18&pause=1000&color=F59E0B&center=true&vCenter=true&random=false&width=650&height=60&lines=Cloud+%26+DevOps+Engineer;terraform+apply+%E2%86%92+no+surprises;Infrastructure+that+doesn't+break" alt="Typing SVG" />

</div>

<div align="center">

**Kartikeya Jain** &nbsp;·&nbsp; Software Engineer (Cloud & DevOps) &nbsp;·&nbsp; Celebal Technologies
&nbsp;·&nbsp; Jaipur, India *(from Dehradun)*

AWS Certified · Azure Administrator Certified · Dean's List × 6

*I build infrastructure that doesn't break - and show up for people the same way.*

</div>

---

## About

I'm a DevOps engineer specialising in cloud infrastructure, currently at **Celebal Technologies** where I provision and automate multi-cloud environments across AWS and Azure for enterprise clients.

My day-to-day: Terraform-based infrastructure provisioning, GitLab CI/CD pipelines, Datadog observability (APM, RUM, Synthetic monitoring), Databricks automation, and solving the kind of real integration problems that don't have a Stack Overflow answer.

Outside of infrastructure - I write. It's how I think and process ideas.

---

## What I Work With

**Cloud & Infrastructure**

![AWS](https://img.shields.io/badge/AWS-232F3E?style=flat-square&logo=amazonaws&logoColor=white)
![Azure](https://img.shields.io/badge/Azure-0078D4?style=flat-square&logo=microsoftazure&logoColor=white)
![Terraform](https://img.shields.io/badge/Terraform-7B42BC?style=flat-square&logo=terraform&logoColor=white)
![Databricks](https://img.shields.io/badge/Databricks-FF3621?style=flat-square&logo=databricks&logoColor=white)
![Docker](https://img.shields.io/badge/Docker-2496ED?style=flat-square&logo=docker&logoColor=white)
![GitLab CI](https://img.shields.io/badge/GitLab_CI-FCA121?style=flat-square&logo=gitlab&logoColor=white)
![Datadog](https://img.shields.io/badge/Datadog-632CA6?style=flat-square&logo=datadog&logoColor=white)

**Infrastructure as Code**

![HCL](https://img.shields.io/badge/HCL-7B42BC?style=flat-square&logo=terraform&logoColor=white)
![Bicep](https://img.shields.io/badge/Bicep-0078D4?style=flat-square&logo=microsoftazure&logoColor=white)
![CloudFormation](https://img.shields.io/badge/CloudFormation-232F3E?style=flat-square&logo=amazonaws&logoColor=white)

**Languages**

![Python](https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white)
![PowerShell](https://img.shields.io/badge/PowerShell-5391FE?style=flat-square&logo=powershell&logoColor=white)

---

## Projects

| Project | What it does | Stack |
|---|---|---|
| [**Terraform IAM Policy Checker**](https://kartikeyajain.com/work/terraform-iam-policy-checker) | VSCode extension - catches IAM policy size limits before `terraform apply` ruins your Friday | VSCode API · AWS IAM · GitHub Actions |
| [**Women's Safety Analysis**](https://kartikeyajain.com/work/women-safety-india) | Data analysis across Indian states 2001-2014 | Python · Pandas · Tableau |
| [**Terrorism Pattern Analysis**](https://kartikeyajain.com/work/terrorism-analysis) | Global terrorism dataset - patterns, hotspots, 41 years of data | Python · Pandas · Tableau |
| [**Cancer Detection KNN**](https://kartikeyajain.com/work/cancer-knn) | KNN classifier - 91.93% accuracy, 95.49% F-Score | Python · scikit-learn |
| [**Titanic Survival Analysis**](https://kartikeyajain.com/work/titanic-analysis) | Survival factor breakdown across passenger classes | Python · Pandas |
| [**CrickView**](https://kartikeyajain.com/work/crickview) | Live cricket stats scraper and dashboard | Python · BeautifulSoup · Streamlit |

---

## Writing

Technical stories, infrastructure lessons, and occasionally - life.

- **[terraform apply. Friday. 7pm. LimitExceeded.](https://kartikeyajain.com/blog/terraform-iam-policy-checker)** - How a ruined Friday plan became an open source VSCode extension
- **[Mastering Terraform on Azure](https://medium.com/@kartikeya47jain/mastering-terraform-on-azure)** - Terraform patterns that actually work in production
- **[A Journey from Click to Code](https://medium.com/@kartikeya47jain/a-journey-from-click-to-code-4a96c8a8309b)** - Why I moved from ClickOps to IaC and never looked back
- More at [kartikeyajain.com/blog](https://kartikeyajain.com/blog)

---

## Research

- **Medical communication: Designing an enhanced health care chatbot for instructive conversations** - AIP Publishing, Jan 2025

---

## GitHub Stats

<div align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=kartikeya47jain&show_icons=true&theme=dark&hide_border=true&include_all_commits=true&count_private=true&title_color=f59e0b&icon_color=f59e0b&text_color=94a3b8&bg_color=0a0f1e" width="48%" />
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=kartikeya47jain&theme=dark&hide_border=true&background=0a0f1e&ring=f59e0b&fire=f59e0b&currStreakLabel=f59e0b&sideLabels=94a3b8&dates=94a3b8" width="48%" />
</div>

---

## Connect

<a href="https://kartikeyajain.com"><img src="https://img.shields.io/badge/Portfolio-0a0f1e?style=flat-square&logo=vercel&logoColor=f59e0b" /></a>
<a href="https://linkedin.com/in/kartikeya-jain-267628220"><img src="https://img.shields.io/badge/LinkedIn-0077B5?style=flat-square&logo=linkedin&logoColor=white" /></a>
<a href="https://twitter.com/KARTIKEYA47JAIN"><img src="https://img.shields.io/badge/Twitter-1DA1F2?style=flat-square&logo=twitter&logoColor=white" /></a>
<a href="https://medium.com/@kartikeya47jain"><img src="https://img.shields.io/badge/Medium-12100E?style=flat-square&logo=medium&logoColor=white" /></a>
<a href="https://kaggle.com/kartikeya47jain"><img src="https://img.shields.io/badge/Kaggle-20BEFF?style=flat-square&logo=kaggle&logoColor=white" /></a>
<a href="mailto:kartikeya007jain@gmail.com"><img src="https://img.shields.io/badge/Email-D14836?style=flat-square&logo=gmail&logoColor=white" /></a>

---

<div align="center">
  <sub>Wind in peak summer.</sub>
</div>
